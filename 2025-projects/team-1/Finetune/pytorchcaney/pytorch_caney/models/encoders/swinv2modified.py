...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            if self.use_checkpoint and any(p.requires_grad for p in blk.parameters()):
                x = checkpoint.checkpoint(blk, x, use_reentrant=True)
                if i >= self.depth - 2: # Capture the last two blocks
                    # Detach to prevent gradient issues if you only need the values
                    # and don't want them to affect backprop through this storage.
                    self.last_block_outputs[f'block_{i}_output'] = x.detach().cpu()
            else:
                x = blk(x)
                if i >= self.depth - 2: # Capture the last two blocks
                    # Detach to prevent gradient issues if you only need the values
    ...
    def get_last_block_statistics(self):
        """
        Calculates mean and standard deviation for the captured last block outputs.
        """
        stats = {}
        for name, tensor in self.last_block_outputs.items():
            # Flatten the tensor to calculate mean/std across all elements
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        #print("swin v2 patch embed forward method !!!!")
        #print(x.shape)
...
class SwinTransformerV2(nn.Module):
    ...
    def get_all_stage_block_statistics(self):
        """
        Collects statistics from the last few blocks of each BasicLayer (stage).
        """
        all_stats = {}
        
        for layer in self.layers:
            stage_stats = layer.get_last_block_statistics()
            if stage_stats:
                all_stats[f'depth {layer.depth}'] = stage_stats
        return all_stats


class PromptedSwinTransformerV2(SwinTransformerV2):
    def __init__(self, prompt_length=10, **kwargs):
        # reminder to myself: doing **kwargs basically just lets you take in all keyword arguments,
        # satvision.py passes in all the arguments to swin as keyword arguments so this will essentially
        # allow us to accept all of them, and also take in the argument prompt_length
        super().__init__(**kwargs)
        # then initialize with the main class's init 

        self.prompt_length = prompt_length

        # Create the learnable prompt.
        # Its size should be # of prompts *  dimension of each patch embedding

        self.prompt_embeddings = nn.Parameter(torch.randn(prompt_length, self.embed_dim))
    ...
    def extra_features(self, x):
        ...
        # prompt_embeddings starts out as P ,  D where P is number of prompts and D is the embed dim.
        # .unsqueeze(0) adds batch dimension -> becomes 1, P, D
        # .expand (B, -1, -1) -> becomes B, P, D
        prompt_embeds = self.prompt_dropout(self.prompt_embeddings.unsqueeze(0).expand(B, -1, -1))
        assert self.prompt_length <= N, "Prompt length must be <= number of patches"
        
        x[:, :self.prompt_length, :] += prompt_embeds
